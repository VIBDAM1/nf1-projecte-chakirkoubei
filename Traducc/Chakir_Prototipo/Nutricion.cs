...
namespace Chakir_Prototipo
{
    public partial class Nutricion : Form
    {
        ...
        private readonly (string codigo, string nombre)[] idiomas =
        {
            ("es", "Español"),
            ("en", "English"),
            ("fr", "Français"),
        ...
        private void ConfigurarMenuIdiomas()
        {
            ...
            ToolStripMenuItem menuIdioma = new ToolStripMenuItem("Idioma");

            // Añadir cada idioma como submenú
            foreach (var idioma in idiomas)
            {
                ToolStripMenuItem itemIdioma = new ToolStripMenuItem(idioma.nombre);
                itemIdioma.Tag = idioma.codigo;
                itemIdioma.Click += CambiarIdioma;
                menuIdioma.DropDownItems.Add(itemIdioma);
            }

            // Añadir el menú de idiomas al menú principal
            menuStrip1.Items.Add(menuIdioma);
        }

        private void CambiarIdioma(object sender, EventArgs e)
        {
            var itemMenu = sender as ToolStripMenuItem;
            string codigoIdioma = itemMenu.Tag.ToString();

            // Aquí implementarás el cambio de idioma más adelante
            MessageBox.Show($"Idioma cambiado a: {itemMenu.Text}\n(Código: {codigoIdioma})",
                          "Idioma seleccionado",
                          MessageBoxButtons.OK,
                          MessageBoxIcon.Information);

            // Más adelante añadirás:
